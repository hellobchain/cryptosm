...
func (e *G1) CombinedMult(H *G1, baseScalar, scalar *big.Int) *G1 {
	...
	e.p.sm9CombinedMult(H.p, baseScalar, scalar)
	return e
}
...
/******************simon add 1220 for homo********************
//copy from zhang
/*
func Test_g2() {
	e1, e2 := &G2{}, &G2{}
	lambda, _ := new(big.Int).SetString("b640000002a3a6eff003ab4ff0477961e1edaee07e84c2d0b978eb1109153e3f", 16)
	...
	e1.p.Mul2(twistGen, lambda)
	y := e1.Marshal()
	fmt.Printf("G2 lambda: %x\n", y)

	e2.p.x.MulScalar(&twistGen.x, beta)
	e2.p.y.Set(&twistGen.y)
	e2.p.z.Set(&twistGen.z)
	e2.p.t.Set(&twistGen.t)
	y2 := e2.Marshal()
	fmt.Printf("G2 Beta: %x\n", y2)

}

func (e *G2) ScalarBaseMult2(k *big.Int) *G2 {
	if e.p == nil {
		e.p = &twistPoint{}
	}
	e.p.Mul2(twistGen, k)
...
func (e *G1) Unmarshal(m []byte) ([]byte, error) {
	...
	if len(m) < 2*numBytes {
		return nil, errors.New("sm9: not enough data")
	}
	...
	if e.p.x == zero && e.p.y == zero {
		...
		if !e.p.IsOnCurve() {
			return nil, errors.New("sm9: malformed point")
		}
	}
