...
//sm2	fffffffe ffffffff ffffffff ffffffff 
...
TEXT ·sm2SelectBase(SB),NOSPLIT,$0
	MOVQ idx+48(FP),AX
	MOVQ table+24(FP),DI
	MOVQ point+0(FP),DX
...
#define hlp BP
/* ---------------------------------------*/
TEXT sm2SubInternal(SB),NOSPLIT | DUPOK,$0
	XORQ mul0, mul0
	SUBQ t0, acc4
	SBBQ t1, acc5
	SBBQ t2, acc6
	...
	RET
/* ---------------------------------------*/
TEXT sm2MulInternal(SB),NOSPLIT | DUPOK,$0
	MOVQ acc4, mul0
	MULQ t0
	MOVQ mul0, acc0
	MOVQ mul1, acc1
	...
	RET
/* ---------------------------------------*/
TEXT sm2SqrInternal(SB),NOSPLIT | DUPOK,$0
...
TEXT ·sm2PointAddAffineAsm(SB),0,$512-96
	...
	MOVQ acc2, t2
	MOVQ acc3, t3
	// Add in case the operand was > sm2-p
	ADDQ $-1, acc0
...
TEXT sm2IsZero(SB),NOSPLIT | DUPOK,$0
...
TEXT ·sm2PointAddAsm(SB),0,$680-80
	// Move input to stack in order to free registers
	MOVQ res+0(FP), AX
	MOVQ in1+24(FP), BX
	MOVQ in2+48(FP), CX
...
TEXT ·sm2PointDoubleAsm(SB),NOSPLIT,$256-48
	// Move input to stack in order to free registers
	MOVQ res+0(FP), AX
	MOVQ in+24(FP), BX
